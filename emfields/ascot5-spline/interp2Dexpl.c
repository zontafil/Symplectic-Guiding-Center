...
int interp2Dexpl_init(interp2D_data* str, realnum* f, int n_r, int n_z,
    ...
    str->c = (realnum*)malloc(n_z*n_r*16*sizeof(realnum));

    /* Declare and allocate the needed variables */
    int i_r;
    int i_z;
    int i_c;
    realnum* f_r = (realnum*)malloc(n_r*sizeof(realnum));
    realnum* f_z = (realnum*)malloc(n_z*sizeof(realnum));
    realnum* c_r = (realnum*)malloc((n_r-1)*4*sizeof(realnum));
    realnum* c_z = (realnum*)malloc((n_z-1)*4*sizeof(realnum));
    int i_s;
    int i_ct;
 ...
 */
a5err interp2Dexpl_eval_B(realnum* B, interp2D_data* str, realnum r, realnum z) {
    int i_r = (r-str->r_min)/str->r_grid;
    realnum dr = (r-(str->r_min+i_r*str->r_grid))/str->r_grid;
    realnum dr2 = dr*dr;
    realnum dr3 = dr2*dr;
    int i_z = (z-str->z_min)/str->z_grid;
    realnum dz = (z-(str->z_min+i_z*str->z_grid))/str->z_grid;
    realnum dz2 = dz*dz;
    realnum dz3 = dz2*dz;
...
a5err interp2Dexpl_eval_dB(realnum* B_dB, interp2D_data* str, realnum r, realnum z) {
    ...
    realnum dr2 = dr*dr;
    realnum dr3 = dr2*dr;
    realnum rgi = 1.0/str->r_grid;
    int i_z = (z-str->z_min)/str->z_grid;
    realnum dz = (z-(str->z_min+i_z*str->z_grid))/str->z_grid;
    realnum dz2 = dz*dz;
    realnum dz3 = dz2*dz;
    realnum zgi = 1.0/str->z_grid;
    int n = i_z*str->n_r*16+i_r*16;

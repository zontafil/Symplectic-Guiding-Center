...
void spline1Dcomp(realnum* f, int n, int bc, realnum* c) {
    ...
    int i; /* index */
    realnum* Y = (realnum*)malloc(n*sizeof(realnum)); /* Array for RHS of matrix equation */
    realnum* p = (realnum*)malloc((n-1)*sizeof(realnum)); /* Array superdiagonal values */
    realnum* D = (realnum*)malloc(n*sizeof(realnum)); /* Array for 2nd derivative vector to solve */

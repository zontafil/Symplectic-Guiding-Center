...
namespace EMFields{
	...
	template <int DIM> class FiniteDFromAB: public GuidingFieldConfiguration<DIM>
	{
		private:
			...
			EMField* field;
		public:
			FiniteDFromAB(Config::Config* config);
			~FiniteDFromAB(){};
	...
	template <int DIM> Vector3d FiniteDFromAB<DIM>::B_grad(Vector3d x){
	...
	template <int DIM> Matrix<double,3,3> FiniteDFromAB<DIM>::B_hessian(Vector3d x){
		// COMPUTE B_HESSIAN
		Vector3d dx(hx,hx,hx); //dx := (dx,dy,dz)
	...
	template <int DIM> GuidingField FiniteDFromAB<DIM>::compute(Matrix<double,DIM/2,1> q){
		//TODO: implement phi
